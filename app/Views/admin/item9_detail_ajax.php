    ...
    <?php if (session()->getFlashdata('msg')) : ?>
	    ...
	    $(document).ready(function() {
			...
			content.url = "<?=base_url('/admin/item9_detail/'.$main.'/'.$sub.'/'.$cate)?>";
...
=======
    		...
    		var file = $('#iu_fileup')[0].files[0];
			var iu_id = $('#iu_id').val();
			var iu_topic = $('#iu_topic').val();
			var iu_typeup = $('#iu_typeup').val();
			var iu_linkurl = $('#iu_linkurl').val();
    		frmdata.append('file',file);
    		frmdata.append('iu_id',iu_id);
    		frmdata.append('iu_topic',iu_topic);
    		frmdata.append('iu_typeup',iu_typeup);
    		frmdata.append('iu_linkurl',iu_linkurl);			
			e.preventDefault();
>>>>>>> 8081b24 (mac up 2022-09-13-1)
    ...
    function deleteItemUpID(dataItMain,dataItSub,dataItCate,dataItArea,dataItSch,dataItID){
        ...
        .then((delUp_Item) => {
            if(delUp_Item){
                location.href = "<?=base_url('admin/delUp_Item')?>/9/" + dataItMain +'/'+ dataItSub +'/'+ dataItCate +'/'+ dataItArea +'/'+ dataItSch +'/'+ dataItID;
            }
        });
    }
